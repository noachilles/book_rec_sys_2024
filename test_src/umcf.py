...
# pip install scikit-surprise 모듈 사용해 user-user memory based cf 구현
        ...
        if is_naive:
            ...
            for user1_id in test_users:
                ...
                for user2_id in user_movie_matrix.index:
                    ...
                    # 공통으로 평가한 아이템이 없는 경우는 스킵한다 - any(): 뭐라도 있으면 1 반환
                    if not common_items.any():
                        continue
                    
                    # 아까는 사용자 각각의 평갓값 벡터 / 지금은 공통 아이템 평갓값 벡터
                ...
                # user_id가 user1의 id와 같으면 해당 행의 rating_pred에 avg_1을 대입
                movie_rating_predict.loc[(movie_rating_predict["user_id"] == user1_id), "rating_pred"] = avg_1

                if similar_users:
                    for movie_id in test_movies:
                        if movie_id in movie_id2index:
                            # similar_users가 index(user_id)가 저장되어 있는 array 이므로
                            # index(user_id)에 대해 column(movie_id)값을 numpy matrix로 만드는 과정
